...
sealed class PacketDecodeException(
  ...
  class InvalidMessageType(raw: Int) : PacketDecodeException("Invalid message type: $raw")
  class MissingRequestPacket(code: UShort) :
    PacketDecodeException("No matching request packet for client, code: $code")

  class Others(cause: Throwable) : PacketDecodeException("Unknown error", cause)
}
...
fun main() {
  ...
  root.walk()
    ...
    .sortedBy {
      val ts = it.name.split('-', limit = 2).getOrNull(0)
...
sealed class Packet {
  class Context(
    ...
    override fun toString(): String {
      return "PacketContext(deque=$deque, len=$len)"
    }
  }
  ...
  companion object {
    ...
    fun parse(stream: InputStream, ctx: Context? = null): Packet = runCatching {
      when (val type = stream.read()) {
        ...
        3 -> {
          ...
          var i = 0
          val entry = ctx.deque.reversed().firstOrNull { i++; it.code == code }
    ...
    @JvmStatic
    fun parseOrNull(stream: InputStream, ctx: Context? = null): Packet? {
      return runCatching {
        parse(stream, ctx)
      }.getOrNull()
    }
